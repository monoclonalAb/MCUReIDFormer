...
class Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # head
        if self.reid_mode:
            # ReID head: bottleneck -> BNNeck -> ID classifier
            self.bottleneck = LinearSuper(embed_dim, feat_dim)
    ...
    def no_weight_decay(self):
        no_decay = {'pos_embed', 'cls_token', 'rel_pos_embed'}
        if self.reid_mode:
            no_decay.add('bn_neck')
        return no_decay
    ...
    def set_sample_config(self, config: dict):
        ...
        if self.reid_mode:
            self.bottleneck.set_sample_config(self.sample_embed_dim[-1], self.feat_dim)
        else:
    ...
    def get_sampled_params_numel(self, config):
        ...
        total = sum(numels) + self.sample_embed_dim[0]* (2 +self.patch_embed_super.num_patches)
        if self.reid_mode:
            total += self.classifier.weight.numel()
            total += self.bn_neck.weight.numel() + self.bn_neck.bias.numel()
        return total
    def get_complexity(self, sequence_length):
        ...
        if self.reid_mode:
            total_flops += self.bottleneck.get_complexity(1)
            total_flops += self.feat_dim * self.num_classes  # classifier
        else:
