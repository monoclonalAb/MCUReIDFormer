...
from supernet_engine import train_one_epoch, evaluate, evaluate_reid
from lib.samplers import RASampler, PKSampler
from lib.losses import ReIDLoss
...
def get_args_parser():
    ...
    parser.add_argument('--num-instances', type=int, default=4,
                        help='Number of images per identity in each batch (K in PK sampling)')
    ...
    parser.add_argument('--data-path', default='./data/imagenet/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', 'REID'],
...
def main(args):
    ...
    if args.reid:
        dataset_train, dataset_query, dataset_gallery, args.nb_classes = build_reid_datasets(args)
        sampler_train = PKSampler(dataset_train, num_instances=args.num_instances,
    ...
    if args.reid:
        criterion = ReIDLoss(smoothing=args.smoothing, margin=args.triplet_margin)
    elif args.mixup > 0.:
    ...
    if args.eval:
        if args.reid:
            test_stats = evaluate_reid(data_loader_query, data_loader_gallery, model, device,
                                       amp=args.amp, mode=args.mode, retrain_config=retrain_config)
            print(f"mAP: {test_stats['mAP']:.4f}  Rank-1: {test_stats['rank1']:.4f}")
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.reid:
            ...
            test_stats = evaluate(data_loader_val, model, device, amp=args.amp, choices=choices,
