...
from lib.datasets import build_dataset, build_reid_datasets
from lib import utils
from supernet_engine import evaluate, evaluate_reid
...
class EvolutionSearcher(object):
    ...
    def __init__(self, args, device, model, model_without_ddp, choices, val_loader, test_loader, output_dir,
                 reid_mode=False, query_loader=None, gallery_loader=None):
        self.device = device
        self.model = model
    ...
    def is_legal(self, cand):
        ...
        if self.reid_mode:
            eval_stats = evaluate_reid(self.query_loader, self.gallery_loader, self.model,
                                       self.device, amp=self.args.amp, mode='retrain',
    ...
    def search(self):
        ...
        while self.epoch < self.max_epochs:
            ...
            self.update_top_k(
                self.candidates, k=self.select_num, key=lambda x: self.vis_dict[x][fitness_key])
            self.update_top_k(
                self.candidates, k=50, key=lambda x: self.vis_dict[x][fitness_key])
            ...
            for i, cand in enumerate(self.keep_top_k[50]):
                if self.reid_mode:
                    print('No.{} {} mAP = {:.4f}, Rank-1 = {:.4f}, params = {}'.format(
                        i + 1, cand, self.vis_dict[cand]['mAP'], self.vis_dict[cand]['rank1'],
                        self.vis_dict[cand]['params']))
                    tmp_accuracy.append(self.vis_dict[cand]['mAP'])
                else:
...
def get_args_parser():
    ...
    # ReID parameters
    parser.add_argument('--reid', action='store_true', help='Enable ReID mode')
    parser.add_argument('--feat-dim', type=int, default=256, help='ReID embedding dimension')
    ...
    parser.add_argument('--data-path', default='/datasets01_101/imagenet_full_size/061417/', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', 'EVO_IMNET', 'REID'],
...
def main(args):
    ...
    data_loader_query = None
    data_loader_gallery = None

    if args.reid:
        _, dataset_query, dataset_gallery, args.nb_classes = build_reid_datasets(args)
        sampler_query = torch.utils.data.SequentialSampler(dataset_query)
        sampler_gallery = torch.utils.data.SequentialSampler(dataset_gallery)
        data_loader_query = torch.utils.data.DataLoader(
            dataset_query, batch_size=int(2 * args.batch_size),
            sampler=sampler_query, num_workers=args.num_workers,
            pin_memory=args.pin_mem, drop_last=False,
        )
        data_loader_gallery = torch.utils.data.DataLoader(
            dataset_gallery, batch_size=int(2 * args.batch_size),
            sampler=sampler_gallery, num_workers=args.num_workers,
            pin_memory=args.pin_mem, drop_last=False,
        )
    else:
    ...
    model = Vision_TransformerSuper(img_size=args.input_size,
                                    ...
                                    reid_mode=args.reid, feat_dim=args.feat_dim)
    ...
    t = time.time()
    searcher = EvolutionSearcher(args, device, model, model_without_ddp, choices,
                                 data_loader_val, data_loader_test, args.output_dir,
                                 reid_mode=args.reid,
                                 query_loader=data_loader_query,
                                 gallery_loader=data_loader_gallery)
