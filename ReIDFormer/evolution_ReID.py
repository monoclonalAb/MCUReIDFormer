"""
Evolution search adapted for Re-Identification
...
class EvolutionSearcherReID(object):
    ...
    def is_legal(self, cand):
        """Check if candidate is legal and evaluate it"""
    ...
    def search(self):
        ...
        print('='*80)
        print('Evolution Search for ReID')
        ...
        while self.epoch < self.max_epochs:
            ...
            print(f'{"Rank":<6} {"mAP":<8} {"R-1":<8} {"R-5":<8} {"Params":<10} {"Config"}')
            print('-' * 80)
            ...
            for i, cand in enumerate(self.keep_top_k[50]):
                ...
                print(f'{i+1:<6} {info["mAP"]:.4f}   {info["rank1"]:.4f}   '
                      f'{info["rank5"]:.4f}   {info["params"]:.2f}M     {str(cand)[:50]}...')
        ...
        print(f'{"="*80}')
        print(f'\nBest architecture (mAP: {self.vis_dict[self.keep_top_k[50][0]]["mAP"]:.4f}):')
...
def get_args_parser():
    parser = argparse.ArgumentParser('Evolution search for ReID', add_help=False)
    ...
    parser.add_argument('--min-param-limits', type=float, default=15)
    ...
    parser.add_argument('--input-size', default=256, type=int) # default=224 for ImageNet
...
def main(args):
    ...
    # Create model
    print(f"Creating SuperVisionTransformer for ReID")
    print(cfg)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            # --- Optional: resume optimizer / scheduler if available ---
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Evolution search for ReID', parents=[get_args_parser()])
