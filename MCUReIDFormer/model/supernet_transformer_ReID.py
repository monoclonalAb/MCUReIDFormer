...
class Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # classifier head (kept for ID loss training)
        self.head = LinearSuper(embed_dim, num_classes) if num_classes > 0 else nn.Identity()

        # RE-ID head: projection -> BNneck -> L2 normalize
        if self.reid:
...
class TransformerEncoderLayer(nn.Module):
    """Encoder layer block with low-rank FFN decomposition."""
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, dropout=0., attn_drop=0.,
        ...
        # Original: fc1 (embed -> ffn_dim), fc2 (ffn_dim -> embed)
        # Low-rank: fc11 -> fc12 -> fc13 (up), fc21 -> fc22 -> fc23 (down)
