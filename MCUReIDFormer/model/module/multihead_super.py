...
class AttentionSuper(nn.Module):
    def __init__(self, super_embed_dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., normalization = False, relative_position = False,
        ...
        # Low-rank QKV decomposition: embed -> rank -> rank -> 3*embed
        ...
        # Low-rank projection decomposition: embed -> rank -> rank -> embed
    ...
    def forward(self, x):
        B, N, C = x.shape
        # Low-rank QKV: x -> qkv1 -> qkv2 -> qkv3
        x_qkv = self.qkv1(x)
        x_qkv = self.qkv2(x_qkv)
        qkv = self.qkv3(x_qkv).reshape(B, N, 3, self.sample_num_heads, -1).permute(2, 0, 3, 1, 4)
        ...
        # Low-rank projection: x -> proj1 -> proj2 -> proj3
        x = self.proj1(x)
        x = self.proj2(x)
        x = self.proj3(x)
        x = self.proj_drop(x)
