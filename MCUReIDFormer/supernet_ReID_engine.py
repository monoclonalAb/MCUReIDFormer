...
def train_one_epoch_reid(model: torch.nn.Module, criterion_id: torch.nn.Module,
                         criterion_triplet: TripletLoss, criterion_center: CenterLoss,
                         data_loader: Iterable, optimizer: torch.optim.Optimizer,
                         center_optimizer: torch.optim.Optimizer, device: torch.device, epoch: int, 
                         max_norm: float = 0, amp: bool = True, choices=None, mode='super', retrain_config=None):
    model.train()
    criterion_id.train()
    ...
    metric_logger.add_meter('loss_tri', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    metric_logger.add_meter('loss_cen', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
...
@torch.no_grad()
def evaluate_reid(query_loader, gallery_loader, model, device, 
...
def extract_features(data_loader, model, device, amp=True):
    ...
    for imgs, batch_pids, img_paths in data_loader:
        ...
        # Parse PID and camid from filename if available
        for path in img_paths:
            basename = os.path.basename(path)
            parts = basename.split("_")
            pid = int(parts[0])
            # The third part (index 2) seems to indicate camera or track
